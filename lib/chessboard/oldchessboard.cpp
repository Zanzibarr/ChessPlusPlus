...
void chessboard::do_enpassant(const coords &_eat, const coords &_start, const coords &_end) {
    ...
    eaten = new empty_tile();
    temp = eaten;
    eaten = board[_eat.first][_eat.second];
...
void chessboard::do_legit(const coords &_start, const coords &_end) {
    ...
    delete eaten;
    eaten = board[_end.first][_end.second];
...
void chessboard::undo(const int _special, const coords &_oth_piece, const coords &_start, const coords &_end, const piece* _eaten) {
    ...
    if (_special != 2) board[_start.first][_start.second] = board[_end.first][_end.second];

    switch(_special) {
        case 0:
            board[_end.first][_end.second] = eaten;
            break;

        case 1:
            board[_oth_piece.first][_oth_piece.second] = eaten;
            break;
...
bool chessboard::check(const set &_side) const {
    ...
    switch(_side) {
        case set::White:
            ...
            /*DA SOSTITUIRE CON IL METODO FIND*/
            for (unsigned int i = 0; i < white.size(); i++) {
    ...
    return is_in_danger(_side, king_coords);
